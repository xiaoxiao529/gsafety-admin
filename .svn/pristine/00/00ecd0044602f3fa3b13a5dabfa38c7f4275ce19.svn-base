    ...
    <div class="main-wrap">
      <el-row class="location">
        ...
        <el-col :span="12" class="add-btn-style">
          <el-button type="primary" icon="el-icon-download" @click="batchExportOutStock" :disabled="tableData.length==0 ?true:false">批量导出excel</el-button>
          <el-button type="success" icon="el-icon-upload2" @click="inStock">入库</el-button>
        </el-col>
      </el-row>
      <div class="main-container">
        ...
        <!-- 列表 -->
        <div class="list-wrap">
          <el-table v-loading.lock="isLoading" ref="table" :data="tableData" stripe show-overflow-tooltip @selection-change="handleSelectionChange">
            <el-table-column
              type="selection"
              width="45" :selectable='selectInit'>
            </el-table-column>
            ...
            <el-table-column label="操作" show-overflow-tooltip align="center" min-width="10%">
              <template slot-scope="scope">
                ...
                <el-link type="primary" @click="setupInfo(scope.$index,scope.row)"  v-if="scope.row.repStatus === 1">安装信息</el-link>
                <el-link type="primary" @click="returnStock(scope.$index,scope.row)"  v-if="scope.row.repStatus === 1">回退入库</el-link>
                <el-link type="primary" @click="sellingStock(scope.$index,scope.row)" v-if="scope.row.repStatus === 0">销库</el-link>
              </template>
            </el-table-column>
          </el-table>
          <div class="page-footer">
            <div class="pf-left">
              ...
              <el-button type="primary" size="mini" @click="batchOutStock" :disabled="multipleSelectionAll.length == 0?true:false">批量出库</el-button>
            </div>
            <div class="pf-right">
              <Pagination
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      batchOutStock(){
        const that = this;
        if(null==that.multipleSelectionAll || that.multipleSelectionAll.length==0){
      ...
      batchExportOutStock(){
        ...
        let url = "/gsafetyclound"+ApiConfig.api['downloadDeviceRepositoryExcelByConditions']+'?'+queryStr;
        that.$confirm('确认批量导出excel？')
          .then(_ => {
            window.open(url)
          })
          .catch(_ => {});
...
<style lang="scss" scoped="scoped">
  ...
  .page-footer{
    ...
    .pf-left{
      ...
      .span-yx{ width: 60px; font-size: 14px; color: #666; display: inline-block}
