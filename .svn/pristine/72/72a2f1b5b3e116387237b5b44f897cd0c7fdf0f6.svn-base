...
<script>
  export default {
    ...
    methods: {
      ...
      findMonitorDeviceStateExtByMonitorId(callback) {
          ...
          .then(res => {
            if (res.data.code == "success") {
              if (callback) {
                ...
                that.updateTime = new Date(res.data.data.updateTime.replace(/-/g, '/')).getTime();
              }
            } else {
              console.log("失败");
            }
      ...
      submitForm() {
        ...
        that.sendCommandHandle(limit);
      },
      sendCommandHandle(limit) {
        let that = this;
        that.sendCommand(5, function () {
          console.log("第" + (limit + 1) + "次调用接口");
          //等待四秒，调用查询接口
          setTimeout(function () {
            that.findMonitorDeviceStateExtByMonitorId(function (res) {
              let timeValue=new Date(res.data.data.updateTime.replace(/-/g, '/')).getTime();
              if (timeValue === that.updateTime) {
                limit++;
                // if(limit==3){//这里是模拟测试成功跳出
                //   that.updateTime=1001;
                // }
                if (limit!== 4) {
                  //继续循环调用
                  that.sendCommandHandle(limit);
