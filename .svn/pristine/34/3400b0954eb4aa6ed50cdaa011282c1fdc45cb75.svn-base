<!--
 * @Descripttion: 运营中心管理--企业用户管--服务机构用户
 * @version: 
 * @Author: yangxq
 * @Date: 2019-07-18 09:48:16
 * @LastEditors: sueRimn
 * @LastEditTime: 2019-08-05 11:42:30
 -->
<template>
  <div>
    <div class="Crumbs">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>服务机构用户</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="main-wrap">
      ...
      <div class="createAdmin-wrap">
        <el-form :inline="true" :model="createAdminForm" :rules="rules" ref="createAdminForm">
          ...
          <el-form-item label="岗位:" prop="station">
            <div class="station-tree-wrap">
              <el-tree
                :data="stationTree"
                show-checkbox
                node-key="id"
                :props="propStation"
                @check-change="handleNodeStation"
              ></el-tree>
            </div>
          </el-form-item>
...
export default {
  ...
  computed: {
    currentOrgcode() {
      return this.treeData.length > 0 ? this.treeData[0].service_unit_id : "";
    },
    genderIcon: function() {
      //详情性别图标
  ...
  mounted() {
    ...
    that.getTtree(() => {
      that.$nextTick(() => {
        setTimeout(function() {
          that.$refs["tree11"].setCurrentKey(that.treeData[0].service_unit_id);
        }, 500);
      });
      //列表;
      that.getTableData();
  ...
  methods: {
    ...
    getTableData() {
      ...
      that.$axios
        .$POST_NEW({
          ...
          params: {
            centerType: 4, //1 总中心2：运营中心 3：业主单位 4：服务机构
    ...
    handleNodeClick(data) {
      let that = this;
      that.orgCode = data.service_unit_id;
      that.getTableData();
      that.getRole();
      //获取岗位树
    },
    //岗位树点击事件
    handleNodeStation(data) {
      if (data.type == "1") {
        that.$message.warning("不可选择部门！");
        return false;
      } else if (data.type == "2" || data.type == "3") {
        that.createAdminForm.postCodes = data.id;
      } else {
        that.$message.warning("只能选择岗位！");
        return false;
      }
    },
    //新增用户
          ...
          that.$axios
            .$POST_NEW({
              ...
              params: {
                ...
                orgCode: that.orgCode || that.currentOrgcode,
                postCodes: that.createAdminForm.postCodes //岗位编码
              }
            })
            .then(res => {
              if (res.data.code == "success") {
    ...
    getStation() {
      ...
      that.$axios
        ...
        .then(res => {
          if (res.data.code == "success") {
            let that = this;
            that.stationTree = res.data.data;
            for (var i = 0; i < that.stationTree.length; i++) {
              ...
              for (var j = 0; j < that.stationTree.length; j++) {
                if (that.stationTree[i].id == that.stationTree[j].parentId) {
                  that.stationTree[i].children.push(that.stationTree[j]);
                }
              }
            }
